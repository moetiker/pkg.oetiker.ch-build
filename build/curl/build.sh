...
PROG=curl       # App name
VER=7.48.0      # App version
PKG=oep/web/curl    # Package name (without prefix)
SUMMARY="$PROG - command line tool for transferring data with URL syntax"
DESC="$SUMMARY"

RUN_DEPENDS_IPS="web/ca-bundle library/security/openssl library/zlib library/libidn"

CONFIGURE_OPTS="--enable-thread --with-ca-bundle=/etc/ssl/cacert.pem"
# curl actually has arch-dependent headers. Boo.
CONFIGURE_OPTS_64="$CONFIGURE_OPTS_64 --includedir=$PREFIX/include/amd64 --disable-ldap"
